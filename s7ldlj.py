...
async def process_pending_commands(key, iv, region):
    ...
    for command in current_commands:
        try:
            ...
            if command['type'] == 'join_only':
            ...
            elif command['type'] == 'join_and_emote':
                # الإدخال والترقيص معاً
                if command['team_code']:
                    print(f"[API] Joining squad: {command['team_code']}")
                    join_packet = await GenJoinSquadsPacket(command['team_code'], key, iv)
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
                ...
                for uid in command['uids']:
                    if uid and uid.isdigit():
                        ...
                        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', emote_packet)
                        await asyncio.sleep(0.3)
                print(f"[API] Join and emote command sent successfully!")
            ...
            while True:
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = f"""[B][C]{get_random_color()}* ♦ By ; S7L ™ DLJ comeantary ♦ *

{get_random_color()}★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★
[FFFFFF]♦ الحالة ♦ ؛ [00FF00]~Online ✓                 ⚠️ by ; S7L TEAM ⚠️
{get_random_color()}★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★ ★"""
                ...
                if data.hex().startswith("120000"):
                    try:
                        ...
                        if inPuTMsG in ("hi", "hello", "fen", "/help"):
                            message = f"""[B][C]{get_random_color()}♣ By : S7L DLJ comeantary BOT ♣
[FFFFFF]♦ تليجيرام ♦ ~ ♣ @dljyta ♣"""
...
async def MaiiiinE():
    Uid, Pw = '4481906989','C415F8D7DCB735D2B2BC52302526B939B3CACEAE2BAB21390050DD0B91789F89'
    ...
    while True:
        try:
            ...
            os.system('clear')
            print(render('S7L TEAM DLJ', colors=['white', 'green'], align='center'))
            print('')
